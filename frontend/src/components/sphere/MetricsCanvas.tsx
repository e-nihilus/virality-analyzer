...
export default function MetricsCanvas({ metrics, autoPlayBeat = true, isPlaying = true }: MetricsCanvasProps) {
  ...
  const [webglError] = useState<string | null>(() => {
    try {
      ...
      if (!testContext) {
        return "Su navegador o tarjeta gráfica no soporta WebGL.";
      }
    } catch {
  ...
  // Mirror interactive UI state into refs so the render loop reads live values
  ...
  // Live prop mirrors read inside the render loop without rebuilding the scene
  const autoPlayBeatRef = useRef<boolean>(autoPlayBeat);
  const isPlayingRef = useRef<boolean>(isPlaying);
  const metricsRef = useRef<MetricState>(metrics);
  ...
  // Keep render-loop mirrors in sync with the latest props (no scene rebuild)
  useEffect(() => {
    autoPlayBeatRef.current = autoPlayBeat;
  }, [autoPlayBeat]);
  ...
  // Setup WebGL Scene
  useEffect(() => {
    // When WebGL is unsupported the canvas host isn't rendered, so this is null.
    ...
    // Dynamic rendering Loop. We accumulate our own animation time so that
    // pausing (isPlaying = false) freezes the shader/rotation without resetting it.
    ...
    const animate = () => {
      ...
      // Real wall-clock delta (always advances) so data-driven values keep
      // converging to the latest metrics even while playback is paused.
      const frameDelta = Math.min(rawDelta, 0.1);
      // Animation delta freezes when paused so motion (spin, beat, internal
      // shader churn) holds still — mirroring the previous sphere's pause.
      const deltaTime = playing ? frameDelta : 0;
      animTime += deltaTime;
      const elapsedTime = animTime;

      // Real-time audio reactive pulse simulation (when activated)
      let beatFrequency = 1.0;
      let beatAmplitude = 1.0;
      if (autoPlayBeatRef.current) {
      ...
      // Interpolate the metric states inside requestAnimationFrame for fluid
      // transitions. Uses the always-advancing frameDelta so the sphere still
      // reflects the latest analysis data even while playback is paused.
      const lerpHero = Math.min(6.0 * frameDelta, 1.0);
      ...
      if (cameraRef.current) {
        const targetZ = 8.5 / zoomRef.current;
      ...
      if (sphere) {
        ...
        const autoSpeedY = autoRotateRef.current ? (0.16 + currentUniformsRef.current.pacing * 0.45) * deltaTime : 0;
      ...
      if (sphere && cameraRef.current && containerRef.current) {
        ...
        // Live orbital readout (driven via DOM to avoid reading refs during render)
        const xRadEl = document.getElementById("hud-xrad");
        const yRadEl = document.getElementById("hud-yrad");
        if (xRadEl) xRadEl.textContent = `X-RAD // ${rotationCurrentRef.current.x.toFixed(3)} RAD`;
        if (yRadEl) yRadEl.textContent = `Y-RAD // ${rotationCurrentRef.current.y.toFixed(3)} RAD`;
        ...
        keys.forEach((key) => {
          ...
          if (labelEl || pathEl || dotEl) {
            ...
            let bx: number, by: number, lx: number, ly: number;
    ...
    // Scene is built once; live values are fed through refs to avoid teardown.
  }, []);

  return (
    <div className="relative w-full h-full min-h-[300px] rounded-2xl overflow-hidden bg-black border border-white/5 shadow-2xl flex items-center justify-center">
